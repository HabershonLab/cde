...
! stop
  ...
  !> StripInactiveMols
  !!
  !! Facilitates removal of inactive molecules from a reaction between `cx1` 
  !! and `cx2`, returning atom masks with only the atoms of active molecules. Here, inactive means
  ...
  !!
  !************************************************************************
  subroutine StripInactiveMols(cx1, cx2, cx1_mask, cx2_mask)
    implicit none
    type(cxs), intent(in) :: cx1, cx2
    logical, dimension(:), allocatable, intent(out) :: cx1_mask, cx2_mask
    logical, dimension(cx1%na) :: keep_atom
    ...
    allocate(cx1_mask(cx1%na))
    allocate(cx2_mask(cx2%na))
...
!      enddo
  ...
  !**********************************************************************************************
  !> GraphsToCoords_Stripped
  !!
  !! Converts a pair of connectivity graphs generated by RunBreakdown to xyz coordinates,
  !! using optimised geometries from a spectator-stripped previous call to 
  !! GraphsToCoords_BD, by optimization under the action of the molecule-molecule COM GRP.
  !!
  !! The optimised geometries of the molecules in wcx_stripped(2) are copied onto their
  !! respective molecules in wcx(2), and only the molecular positions are optimised without 
  !! changing molecular geometries.
  !!
  !! wcx - Working cx list for full (non-stripped) system
  !! wcx_stripped - Working cx list for system stripped by StripInactiveMols.
  !! wcx1_mask - Mask from StripInactiveMols indicating which atoms from wcx(1) are present in wcx_stripped(1)
  !! wcx2_mask - Mask from StripInactiveMols indicating which atoms from wcx(2) are present in wcx_stripped(2)
  !! printflag - Logical flag indicating whether to print out xyz files or not.
  !! printfile - If printflag = .TRUE., printfile indicates the file to print to.
  !! err - Error flag.
  !! errstr - Error message, if err == .true.
  ...
  !**********************************************************************************************
  !
  Subroutine GraphsToCoords_Stripped(wcx, wcx_stripped, wcx1_mask, wcx2_mask, &
      & printflag, printfile, err, errstr)
    implicit none
    character(len=*), intent(in) :: printfile
    logical, intent(in) :: printflag
    type(cxs), dimension(2), intent(in) :: wcx_stripped
    logical, dimension(:), intent(in) :: wcx1_mask, wcx2_mask
    type(cxs), dimension(2), intent(inout) :: wcx
    logical, intent(out) :: err
    character(len=100), intent(out) :: errstr
    ...
    type(cxs) :: wcx_temp
    logical :: success
    integer :: j, k, natoms, isum, i, irxn, counter
    integer :: stripped_mol_last, stripped_mol, full_mol_last, full_mol
    integer, dimension(:), allocatable :: atom_map
    integer, dimension(:, :), allocatable :: grstore
    real(8), dimension(:, :), allocatable :: rstore
    real(8), dimension(3) :: com_stripped_mol, com_full_mol
    real(8) :: x, y, z
    real(8) :: err_real_blank, grp_zero
    ...
    ! Apply optimised positions of stripped molecules onto
    ! their positions in the full system by translating wrt
    ! their COMs in the stripped and full representations.
    ! 
    ! Map atoms in wcx_stripped(2) to atoms in wcx(2).
    ! e.g. atom_map(stripped_idx) -> full_idx
    allocate(atom_map(wcx_stripped(2)%na))
    counter = 0
    do i = 1, wcx(2)%na
      if (wcx2_mask(i)) then
        counter = counter + 1
    ...
    ! Obtain transformed coords in wcx_temp.
    ! Temporary accumulation is needed so COMs of full mols
    ! aren't modified during transformations, producing 
    ! distorted geometries that go into the COM GRP.
    call CopytoNewCXS(wcx(2), wcx_temp)
    do i = 1, wcx_stripped(2)%na
      com_stripped_mol = MolecularCOM(wcx_stripped(2), MolIdOfAtom(i, wcx_stripped(2)))
      com_full_mol = MolecularCOM(wcx(2), MolIdOfAtom(atom_map(i), wcx(2)))

      wcx_temp%r(1, atom_map(i)) = wcx_stripped(2)%r(1, i) - com_stripped_mol(1) + com_full_mol(1)
      wcx_temp%r(2, atom_map(i)) = wcx_stripped(2)%r(2, i) - com_stripped_mol(2) + com_full_mol(2)
      wcx_temp%r(3, atom_map(i)) = wcx_stripped(2)%r(3, i) - com_stripped_mol(3) + com_full_mol(3)
    end do

    ! Copy transformed coords back to wcx(2)
    call CopyCXS(wcx_temp, wcx(2))
    call SetCXSconstraints(wcx(2), NDOFconstr, FixedDOF, Natomconstr, FixedAtom)
    call GetMols(wcx(2))

    ! Optimize under GRP, only moving under molecular COMs.
    call OptimizeGRPCOM(wcx(2), success, kradius*100, ngdsrelax, gdsdtrelax)

    if (.not. success) then
      err = .TRUE.
      errstr = 'Optimisation of product under COM-only GRP failed'
      wcx(2)%r(:, :) = rstore(:, :)
      wcx(2)%graph(:, :) = grstore(:, :)
      call GetMols(wcx(2))
      return
    ...
    return
  end Subroutine GraphsToCoords_Stripped
  ...
  subroutine RunBreakdown()
    ...
    logical, dimension(:), allocatable :: atomchange, wcx1_mask, wcx2_mask
    logical, dimension(:, :), allocatable :: bondchange
    real(8) :: err_real_blank
      ...
      rxloop: do istep = 1, nrxn
        ...
        stepgen: do while (cyc)
          ...
          if (stripinactive) then
            ...
            call StripInactiveMols(wcx(1), wcx(2), wcx1_mask, wcx2_mask)

            ! Generate new cxs for the reduced systems.
            call CreateCXSFromMask(wcx(1), wcx1_mask, wcx_stripped(1))
            call CreateCXSFromMask(wcx(2), wcx2_mask, wcx_stripped(2))
            if (wcx_stripped(1)%na .ne. wcx_stripped(2)%na) then
              print *, "Number of atoms not conserved by StripInactiveMols."
              print *, "CX1: ", wcx_stripped(1)%na, " CX2: ", wcx_stripped(2)%na
              stop
            endif
            ...
            if (istep .lt. nrxn) then
              ...
              call GraphsToCoords_Stripped(wcx, wcx_stripped, wcx1_mask, wcx2_mask, &
                  & .false., "", errflag, errstr)
            endif

            deallocate(wcx1_mask)
            deallocate(wcx2_mask)
          else
            if (optaftermove) write(logfile, '("- Optimising reaction step...")')
